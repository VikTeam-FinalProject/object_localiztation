...
def lost(feats, dims, scales, init_image_size, k_patches=100, dynamic_thres=False, dbscan=True, add_thres=0):
    """
    ...
    sorted_patches, scores, jumps = patch_scoring(A, dynamic_thres, k_patches=k_patches, add_thres=add_thres)
    ...
    return np.asarray(pred), A, scores, not_potentials_filtered_index, jumps, len(sorted_patches)


def loop_thres(feats, dims, scales, init_image_size, k_patches=100, dynamic_thres=False, dbscan=True, add_thres=0):
    if add_thres == 0:
        return [lost(feats, dims, scales, init_image_size, k_patches, dynamic_thres, dbscan, add_thres)]

    fake_thres = [ft for ft in range(0, add_thres + 1, int(add_thres / 8))]
    thres_tmp = [lost(feats, dims, scales, init_image_size, k_patches, dynamic_thres, dbscan, thres) for thres in
                 fake_thres]
    return thres_tmp
...
def patch_scoring(M, dynamic_threshold, k_patches, add_thres=0):
    """
    ...
    """
    # add_thres = -150
    threshold = torch.mean(M) + add_thres if dynamic_threshold else 0.0
    ...
    if k_patches == -1:
        ...
        num_10_percent = int(len(jumps) * 0.025)
        ...
        #plot cent
        plt.plot(cent.cpu().numpy())

        return sel[:k_jump], cent, jumps

    return sel, cent, None
