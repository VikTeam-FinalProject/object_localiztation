...
def w12(blk, x):
    ...
    # return F.silu(x1) * x2
    if hasattr(blk.mlp, "w12"):         # gated version
        def mlp_first(x):
            x1, x2 = blk.mlp.w12(x).chunk(2, dim=-1)
            return F.silu(x1) * x2
    else:                               # standard fc1 + GELU
        def mlp_first(x):
            return F.gelu(blk.mlp.fc1(x), approximate="none")
...
def anomaly_dir_mlp_ls(
    ...
    with torch.no_grad():
        ...
        if hasattr(blk.mlp, "w3"):          # original SINDER / DeiT-gated style
            A3 = blk.mlp.w3.weight
            B3 = blk.mlp.w3.bias
        elif hasattr(blk.mlp, "fc2"):       # timm / DINO-v2 style
            A3 = blk.mlp.fc2.weight
            B3 = blk.mlp.fc2.bias
        else:
            raise AttributeError("Unknown MLP layout – neither w3 nor fc2 found")
        M = A3.shape[1]
        X = torch.randn(100000, N, device=dev)
        def hidden_activation(x):
            if hasattr(blk.mlp, "w12"):          # gated SiLU variant (original SINDER/DeiT-g)
                x1, x2 = blk.mlp.w12(x).chunk(2, dim=-1)
                return F.silu(x1) * x2
            else:                                # timm / DINO-v2: fc1 + GELU
                return F.gelu(blk.mlp.fc1(x), approximate="none")

        Y = hidden_activation(X)                 #  Tensor (100 000, H)
        #Y = w12(blk, X)
        if bias_ls:
