...
from object_discovery import lost, detect_box, dino_seg,loop_thres
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "dinov2_vits14_reg4_pretrain",
            "dinov2_vitg14_reg4_pretrain",
        ],
    ...
    parser.add_argument("--add_thres", type=int,default=0)
    # Visualization
    parser.add_argument(
        "--visualize",
        type=str,
    ...
    for im_id, inp in enumerate(pbar):
        ...
        if not args.dinoseg:
            thres = loop_thres(
                ...
                add_thres=args.add_thres
            )
            pred, A, scores, seed, potentials, jumps = thres[0]
            ...
            if args.visualize == "fms":
                ...
                image = dataset.load_image(im_name)
                if args.add_thres is not None:
                    fake_thres = [ft for ft in range(0, args.add_thres+1, int(args.add_thres / 8))]
                    for t, value in zip(thres,fake_thres):
                        pred, A, scores, potentials, jumps, k = t
                        plot_score = True if value == args.add_thres else False
                        visualize_predictions(image, pred, scales, [w_featmap, h_featmap], vis_folder, im_name,
                                              plot_score=plot_score, potentials=potentials, jumps=jumps, add_thres=value,scores=scores,k=k)
                else:
                    visualize_predictions(image, pred, scales, [w_featmap, h_featmap], vis_folder, im_name,
                                          plot_score=True, potentials=potentials,jumps=jumps)
        ...
        if torch.any(ious >= 0.5):
            ...
            missing_img.append(im_name)

        cnt += 1
        if cnt % 50 == 0:
            pbar.set_description(f"Found {int(np.sum(corloc))}/{cnt}")
    with open("missing_image.txt", "w") as file:
        for line in missing_img:
            file.write(line + "\n")
