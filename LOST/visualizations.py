# Copyright 2021 - Valeo Comfort a nd Driving Assistance - Oriane Siméoni @ valeo.ai
#
...
def visualize_predictions(image, pred, scales, dims, vis_folder, im_name, plot_score=False, potentials=None, jumps=None, add_thres=0,scores=None,k=0):
    """
    ...
    if potentials is not None:
        ...
        os.makedirs(f"{vis_folder}/{im_name}", exist_ok=True)
        pltname = f"{vis_folder}/{im_name}/add_{add_thres}_potentials.png"
        Image.fromarray(image_copy).save(pltname)
        print(f"Predictions saved at {pltname}.")
        # save jumps plot
        ...
        # plt.grid(True)
        os.makedirs(f"{vis_folder}/{im_name}/scores", exist_ok=True)
        scores = scores.cpu().numpy()
        plt.plot(scores)
        plt.title('Plot of Scores')
        plt.xlabel('Index')
        plt.ylabel('Score Value')
        plt.plot(k, scores[k], 'ro')
        if plot_score:
            plt.savefig(f"{vis_folder}/{im_name}/scores/jumps_add_{add_thres}.png")
